...
virus* readVirus(FILE* input){
    ...
    if(bigEndian) vir->SigSize = size[1] + size[0]*16*16;
    else  vir->SigSize = size[0] + size[1]*16*16;
...
/*
link* pre_append(link* virus_list, virus* data){
    link* lastLink = (link*) malloc(sizeof(link));
    lastLink->vir = data;
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    char fixedBytes[signitureSize];
    FILE* infected;
    for(int i = 0; i < signitureSize; i++) fixedBytes[i] = 0x90;
    if ((infected = fopen(fileName, "r+")) == NULL){
        fprintf(stderr, "infected file not found!\n");
        return;
    }
    fseek(infected, signitureOffset, SEEK_SET);
    fwrite(fixedBytes, signitureSize, 1, infected);
    fclose(infected);
}
...
void Load_signatures(){
    ...
    if(memcmp(endianLetters, "VISB", 4) == 0) bigEndian = 1;
...
void Fix_file(){
    ...
    char offset[100], size[100];
    fprintf(stderr, "Enter virus offset:\n");
    fgets(offset, 100, stdin);
    sscanf(offset, "%d", &signitureOffset);
    fprintf(stderr, "Enter virus size:\n");
    fgets(size, 100, stdin);
    sscanf(size, "%d", &signitureSize);
    kill_virus(infectedName, signitureOffset, signitureSize);
}

struct fun_desc menu[] = { { "Load signatures", Load_signatures }, { "Print signatures", Print_signatures }, { "Detect viruses", Detect },  { "Fix file", Fix_file }, { NULL, NULL } };
